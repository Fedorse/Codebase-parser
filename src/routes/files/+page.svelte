<script lang="ts">
    ...
    const openDialogCode = async (file)=>{
        selectedFile = file
        ...
        <Card.Root class='max-w-sm'>
          <Card.Header>
            ...
            />
                ...
                {:else}
              <span class="flex-1 max-w-64 truncate">{file.name}</span>
              <Button
                variant="link"
                size="sm"
                onclick={() => {
          ...
          <Card.Content class='min-h-80 overflow-hidden'>
            <p class="line-clamp-[13] text-muted-foreground">{file.preview}</p>
          </Card.Content>
          <Card.Footer class='flex justify-between w-full border-t border-text-muted-foreground/20  '>
            <div>
                <Tabs.Root value="preview" >
                    <Tabs.List>
                      <Tabs.Trigger value="preview">Prewiew</Tabs.Trigger>
                      <Tabs.Trigger value="code" onclick={()=>openDialogCode(file)}>Code <Code/></Tabs.Trigger>
                    </Tabs.List>
                  </Tabs.Root>
            </div>
            <div class="flex"> 
              <Button variant='link' onclick={()=> handleOpenDir(file)}><OpenFolder/></Button>
